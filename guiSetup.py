...
def set_up_list(view):
    ...
    view.listbox = Listbox(view.list_frame, yscrollcommand=scrollbar.set)
...
def set_up_menu(view):
    ...
    filemenu.add_command(label="Physikalische Größe definieren", command=view.add_physical_quantity)
    filemenu.add_command(label="Physikalisches Gesetz definieren", command=view.add_physical_law)
    filemenu.add_command(label="Physikalischen Begriff definieren", command=view.add_physical_term)
    filemenu.add_command(label="Einheit definieren", command=view.add_unit)
...
def input_window(root, name_dict, title="Eingabefenster"):
    ...
    for i, name in enumerate(name_dict):
        ...
        if type(name_dict[name]) is StringVar:
            e=Entry(toplevel, textvariable=name_dict[name], width=50)
            e.grid(row=i, column=1, padx=5, pady=5)
            e.bind("<Control-KeyRelease-a>", lambda event, e=e: e.select_range(0, END))
        elif type(name_dict[name]) is BooleanVar:
            Checkbutton(toplevel, variable=name_dict[name]).grid(row=i, column=1)
    Button(toplevel, text="OK", command=toplevel.destroy).grid(row=i+1, column=0)
    toplevel.bind("<Return>", lambda event: toplevel.destroy())
